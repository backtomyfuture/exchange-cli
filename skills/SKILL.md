...
- `email send` 与 `draft create`：支持 `--attach <path>` 携带普通文件附件（可多传），支持 `--inline-attach <path[:cid]>` 携带内联附件（可多传，省略 `:cid` 默认取文件名；配合 `--body-type html` 可在正文中以 `<img src="cid:xxx">` 引用且 Exchange 服务端不显示顶部回形针）；支持 `--cc` 抄送收件人；均支持 `--bcc` 密送收件人；均支持 `--from <addr>` 显式绑定发件人（author / Send-As）；均支持 `--body-type [text|html]`（默认 text）。草稿 HTML 正文默认自动进行防崩溃清洗（移除 ProgId/Generator meta、Edit-Time-Data link、残缺 MSO 条件注释、Office XML 标签与空 @font-face，避免 Outlook 双击编辑打开变空白），支持 `--no-sanitize` 显式跳过。
- `email reply` 与 `email forward`：支持 `--draft` 将原生回复/转发直接保存至草稿箱（自动由服务端拼接历史邮件引用头、维护会话树，返回新建草稿 ID，免 `--confirm`，并自动对保存后的草稿正文做防崩溃清洗，支持 `--no-sanitize`）；均支持 `--body-type [text|html]`；`email reply` 还支持 `--from <addr>`。
