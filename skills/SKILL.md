...
version: 1.1.0
description: |
  Exchange/Outlook 邮件与办公 CLI 工具：收发邮件、搜索邮件、管理草稿、查看和创建日历事件、管理任务待办、查询联系人。
  ...
  回复或转发邮件、搜索特定邮件、创建或查看日历会议、管理任务待办、查找联系人。
  即使用户只是说"帮我查一下邮件"、"发一封邮件给xxx"、"看看今天有什么会议"、"帮我建个会议"、
  "把这个任务标记为完成"、"找一下张三的邮箱"，也应该触发。
...
返回的详细邮件还额外包含 `body`（完整 HTML 正文）、`bcc`、`attachments` 数组。
